...
class User extends MY_Controller {
	...
	function add_user()
	{
		...
		if ($this->form_validation->run() == TRUE)
		{
			...
			$salt_password = $this->bcrypt->hash_password($password);
			$data = array (
				'role_id'=>$role,
	...
	function ajax_delete_user()
	{
		...
		## get all user data ##
		$query = $this->user_model->get_by('staff_id',$staff_id);
		
		## check whether this user is superior ##
		if($query->role_id == 2){
			## if superior, then update assigned this superior to admin in charge  ##
			$this->user_model->update_by(array('superior_id'=>$staff_id), array('superior_id'=>1));
		}
	...
	function edit_user()
	{
		...
		if ($this->form_validation->run() == TRUE)
		{
			...
			if($this->input->post('password'))
			{
				...
				$salt_password = $this->bcrypt->hash_password($password);
				$password = $this->encode($password);
				$data = array('password'=>$password,'salt'=>$salt_password) + $data ;
			}
	...
	function ajax_delete_user_position()
	{
		...
		$query = $this->user_model->view_valid_position($position_id)->get_all();
